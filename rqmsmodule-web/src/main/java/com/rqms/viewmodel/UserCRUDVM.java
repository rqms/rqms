...
public class UserCRUDVM extends UserProfileListVM {
	...
	@Command
	public void save(@BindingParam("action") Integer action) {

		if (MyLib.IsValidBean(this.selectedRecord) == false) {
			return;
		}
		
		CRUDService.Save(selectedRecord);
	...
	/**
	 * Refressh Parent List
	 */
	public void refreshParentList() {
		Binder bind = null;
		if(parentView !=null)
		bind = (Binder) parentView.getAttribute("binder");
		if (bind == null)
			return;
		bind.postCommand("refreshList", null);
	}
